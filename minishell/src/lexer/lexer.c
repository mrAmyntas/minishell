...
/*   Updated: 2022/02/24 16:38:23 by bhoitzin      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/minishell.h"

//j is strlen line_read
static char	*ft_remove_spaces(t_info *info, char *line_read, int i, int j)
{
	char	*buff;
	...
	if (buff == NULL)
		ft_error(info, -1);
	j = 0;
	while (line_read[i + j] != '\0')
	{
		buff[j] = line_read[i + j];
		j++;
	}
	buff[j] = '\0';
	free(line_read);
